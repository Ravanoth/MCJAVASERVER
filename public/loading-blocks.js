...
(function () {
  ...
  function texture(block, face) {
    ...
    if (block.name === 'magenta' || block.name === 'purple' || block.name === 'teal' || block.name === 'blue') {
      ctx.fillStyle = 'rgba(255,255,255,.18)';
      for (let i = 0; i < 18; i++) ctx.fillRect((i * 11) % 32, (i * 7) % 28, 2, 2);
    } else {
  ...
  style.textContent = `
    ...
    .mc-loading-cube {
      ...
      margin-left: var(--cube-offset);
      position: relative;
      transform-style: preserve-3d;
      animation: mcLoadingSpin 3.8s cubic-bezier(.65,0,.35,1) infinite;
    ...
    .mc-loading-face {
      ...
      border: 1px solid rgba(205, 205, 205, .72);
    ...
    .mc-loading-label {
      ...
      animation: mcLoadingText 3.8s ease-in-out infinite;
    }
    @keyframes mcLoadingSpin {
      ...
      40% {
        transform: rotateX(12deg) rotateY(144deg) rotateZ(-4deg);
        filter: saturate(1.7) brightness(1.25);
      }
      60% {
        transform: rotateX(-8deg) rotateY(216deg) rotateZ(4deg);
        filter: saturate(1.4) brightness(1.18);
      }
      80% {
        transform: rotateX(12deg) rotateY(288deg) rotateZ(-4deg);
    ...
    @media (prefers-reduced-motion: reduce) {
      .mc-loading-cube, .mc-loading-label { animation-duration: 12s; }
    }
  `;
